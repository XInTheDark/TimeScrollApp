...
enum HEVCFrameExtractor {
    ...
    static func image(forPath path: URL, startedAtMs: Int64, format: String, maxPixel: CGFloat? = nil) -> NSImage? {
        guard let access = VaultMediaAccess.token(for: path) else { return nil }
        let image = loadImage(forPath: path, startedAtMs: startedAtMs, format: format, maxPixel: maxPixel)
        return VaultMediaAccess.isCurrent(access, for: path) ? image : nil
    }

    private static func loadImage(forPath path: URL, startedAtMs: Int64, format: String, maxPixel: CGFloat?) -> NSImage? {
        ...
        if ext == "mov" {
            ...
            let live = path.deletingPathExtension().appendingPathExtension("mov")
            if FileManager.default.fileExists(atPath: path.path) {
                if let cg = copyFrame(fromMOV: path, offsetMs: offset) { return downsampleCG(cg, maxPixel: maxPixel) }
    ...
    private static func copyFrame(fromMOV url: URL, offsetMs: Int64) -> CGImage? {
        guard let entry = HEVCAssetCache.shared.entry(for: url) else { return nil }
        return withExtendedLifetime(entry) { copyFrame(from: entry.asset, offsetMs: offsetMs) }
    }

    private static func copyFrame(from asset: AVURLAsset, offsetMs: Int64) -> CGImage? {
